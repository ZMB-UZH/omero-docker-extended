...
class ImportPluginRegressionTests(TestCase):
    ...
    def test_normalize_sem_edx_associations_filters_invalid_entries_and_deduplicates_txt(self):
        normalized_entries = [
            {
                "relative_path": "samples/image_a.tif",
                "staged_path": "_staged/samples/image_a.tif",
            },
            {
                "relative_path": "samples/report_a.txt",
                "staged_path": "_staged/samples/report_a.txt",
            },
            {
                "relative_path": "samples/report_b.txt",
                "staged_path": "_staged/samples/report_b.txt",
            },
            {
                "relative_path": "samples/notes.csv",
                "staged_path": "_staged/samples/notes.csv",
            },
        ]
        ...
        normalized = core_functions._normalize_sem_edx_associations(
            {
                "samples/image_a.tif": [
                    "samples/report_a.txt",
                    "samples/report_b.txt",
                    "samples/report_a.txt",
                    "samples/notes.csv",
                    "../escape.txt",
                    "",
                ],
                "samples/report_a.txt": ["samples/report_b.txt"],
                "samples/missing.tif": ["samples/report_a.txt"],
            },
            normalized_entries,
        )
    ...
    def test_build_sem_edx_associations_from_entries_groups_by_directory_with_stable_image_choice(self):
        associations = core_functions._build_sem_edx_associations_from_entries(
            [
                {"relative_path": "sample-dir/z-image.tif"},
                {"relative_path": "sample-dir/a-image.tif"},
                {"relative_path": "sample-dir/02-spectrum.txt"},
                {"relative_path": "sample-dir/01-spectrum.txt"},
                {"relative_path": "txt-only/lonely.txt"},
                {"relative_path": "sample-dir/../escape.txt"},
                {"relative_path": 123},
                "not-a-dict",
            ]
        )
        ...
        self.assertEqual(
            {
                "sample-dir/a-image.tif": [
                    "sample-dir/01-spectrum.txt",
                    "sample-dir/02-spectrum.txt",
                ]
            },
            associations,
        )

    def test_extract_import_candidates_ignores_metadata_and_non_path_lines(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_root = Path(tmpdir)
            candidate = tmp_root / "group" / "image.ome.tif"
            candidate.parent.mkdir(parents=True, exist_ok=True)
            candidate.write_text("pixels", encoding="utf-8")

            output = "\n".join(
                [
                    "2 file(s) parsed into 1 group(s) with 1 call(s) to setId",
                    f"# Group: {candidate.parent} SPW: false",
                    "Reader: ZeissCZIReader",
                    "Would import 2 file(s)",
                    "file: /tmp/not-a-candidate.tif",
                    "relative/path/file.tif",
                    f'"{candidate}"',
                ]
            )

            candidates = core_functions._extract_import_candidates(output)

            self.assertEqual([str(candidate)], candidates)

    def test_has_import_candidates_in_output_accepts_matching_group_root(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_root = Path(tmpdir)
            expected_group_root = tmp_root / "plate.zarr"
            other_candidate = tmp_root / "other" / "image.tif"
            expected_group_root.mkdir(parents=True, exist_ok=True)
            other_candidate.parent.mkdir(parents=True, exist_ok=True)
            other_candidate.write_text("pixels", encoding="utf-8")

            output = "\n".join(
                [
                    f"# Group: {expected_group_root} SPW: false",
                    str(other_candidate),
                ]
            )
            ...
            self.assertTrue(
                core_functions._has_import_candidates_in_output(
                    output,
                    expected_file_path=expected_group_root,
                )
            )
            self.assertFalse(
                core_functions._has_import_candidates_in_output(
                    output,
                    expected_file_path=tmp_root / "missing.zarr",
                )
            )

    def test_write_cli_ice_config_merges_base_config_and_keepalive(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            cli_home = Path(tmpdir)
            base_config = cli_home / "base-ice.config"
            base_config.write_text(
                "Ice.Warn.Connections=1\n\n",
                encoding="utf-8",
            )

            target_path = core_functions._write_cli_ice_config(
                cli_home,
                keepalive_seconds=120,
                base_config_path=str(base_config),
            )

            self.assertEqual(cli_home / "omero-cli-ice.config", target_path)
            self.assertEqual(
                "Ice.Warn.Connections=1\nomero.keep_alive=120\n",
                target_path.read_text(encoding="utf-8"),
            )
            self.assertEqual(0o600, target_path.stat().st_mode & 0o777)

    def test_run_local_import_scan_uses_isolated_cli_environment_and_cleans_omerodir(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_root = Path(tmpdir)
            upload_root = tmp_root / "upload-root"
            compat_root = tmp_root / "compat-root"
            upload_root.mkdir(parents=True, exist_ok=True)
            compat_root.mkdir(parents=True, exist_ok=True)
            sample_path = tmp_root / "sample.ome.tif"
            sample_path.write_text("pixels", encoding="utf-8")
            fake_uuid = types.SimpleNamespace(hex="abcdef1234567890")
            seen = {}

            def fake_run(cmd, capture_output, text, check, timeout, env, stdin):
                seen["cmd"] = cmd
                seen["timeout"] = timeout
                seen["env"] = dict(env)
                Path(env["OMERODIR"]).mkdir(parents=True, exist_ok=True)
                return subprocess.CompletedProcess(
                    args=cmd,
                    returncode=0,
                    stdout="scan-output",
            ...
            with mock.patch.object(core_functions, "_get_upload_root", return_value=upload_root), mock.patch.object(
                ...
                result = core_functions._run_local_import_scan(sample_path, timeout=321)

            expected_omerodir = compat_root / f"{os.getpid()}-abcdef12"
            cli_home = upload_root / ".omero-cli-home"
            ...
            self.assertEqual(
                [
                    ...
                    "-f",
                    "--depth",
                    str(core_functions.OMERO_IMPORT_SCAN_DEPTH),
                    str(sample_path),
                ],
                seen["cmd"],
            )
            self.assertEqual(321, seen["timeout"])
            self.assertEqual(str(expected_omerodir), seen["env"]["OMERODIR"])
            self.assertEqual(str(cli_home), seen["env"]["HOME"])
            self.assertEqual(str(cli_cache), seen["env"]["XDG_CACHE_HOME"])
            self.assertTrue(cli_home.is_dir())
            self.assertTrue(cli_cache.is_dir())
            self.assertFalse(expected_omerodir.exists())
...
class ManageZarrManagedRepositoryScriptTests(TestCase):
    ...
    def test_load_server_config_reads_shared_tmp_from_runtime_state(self):
        manage_script = _load_manage_zarr_script_module()

        with tempfile.TemporaryDirectory() as tmpdir:
            omerodir = Path(tmpdir) / "OMERODIR"
            state_path = omerodir / "var" / "managed-zarr-runtime.env"
            state_path.parent.mkdir(parents=True, exist_ok=True)
            state_path.write_text(
                "# comment\nomero.web.import.shared_tmp_path=/srv/omero/shared-tmp\n",
                encoding="utf-8",
            )

            values = {
                manage_script._CONFIG_DATA_DIR: "/srv/omero",
                manage_script._CONFIG_MANAGED_DIR: "ManagedRepository",
                manage_script._CONFIG_REPO_PATH: "%group%/%user%/%year%-%month%-%day%/%time%",
            }
            ...
            conn = types.SimpleNamespace(
                c=types.SimpleNamespace(
                    sf=types.SimpleNamespace(
                        getConfigService=lambda: config_service
                    )
                )
            )

            with mock.patch.dict(os.environ, {"OMERODIR": str(omerodir)}, clear=False):
                config = manage_script._load_server_config(conn)

            self.assertEqual("/srv/omero", config[manage_script._CONFIG_DATA_DIR])
            self.assertEqual("ManagedRepository", config[manage_script._CONFIG_MANAGED_DIR])
            self.assertEqual(
                "%group%/%user%/%year%-%month%-%day%/%time%",
                config[manage_script._CONFIG_REPO_PATH],
            )
            self.assertEqual(
                "/srv/omero/shared-tmp",
                config[manage_script._CONFIG_SHARED_TMP_PATH],
            )

    def test_render_repo_template_rejects_unknown_tokens(self):
        manage_script = _load_manage_zarr_script_module()

        with self.assertRaisesRegex(RuntimeError, "unsupported tokens"):
            manage_script._render_repo_template(
                {
                    "omero.fs.repo.path": "%group%/%user%/%unknown%",
                },
                "users_private",
                "test",
    ...
    def test_render_repo_template_requires_user_token(self):
        manage_script = _load_manage_zarr_script_module()

        with self.assertRaisesRegex(RuntimeError, "%user% token"):
            manage_script._render_repo_template(
                {
                    "omero.fs.repo.path": "%group%/%year%-%month%-%day%",
                },
                "users_private",
                "test",
                real_datetime(2026, 3, 22, 9, 51, 15),
    ...
    def test_cleanup_zarr_refuses_to_delete_user_prefix_root(self):
        manage_script = _load_manage_zarr_script_module()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(RuntimeError, "Refusing to delete the user managed-repository prefix"):
