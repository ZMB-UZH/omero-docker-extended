...
class CavemanSkillContractTests(unittest.TestCase):
    ...
    def test_active_caveman_skill_is_guarded_and_repo_specific(self) -> None:
        ...
        self.assertIn("Follow an explicit reply-language instruction first", skill_text)
        self.assertIn("otherwise preserve the user's dominant language", skill_text)
    ...
    def test_upstream_reference_records_latest_reviewed_release(self) -> None:
        ...
        self.assertIn("observed and reviewed on 2026-09-19", upstream_text)
        self.assertIn("`v2.7.0`", upstream_text)
