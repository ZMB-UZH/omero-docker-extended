...
Inside this repository, do not create or edit live-checkout `.cocoindex_code/`
settings. Use `python3 tools/cocoindex_agent_search.py`, which owns external
settings under `AGENT_COCOINDEX_HOME` or XDG data paths.

For upstream native use outside this repository, configuration lives in two YAML
files, both created automatically by `ccc init`.
...
- After editing native settings outside this repository, run `ccc index` to
  re-index with the new settings.
