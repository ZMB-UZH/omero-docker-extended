...
6. If GitHub or DeepSource auth is required and no valid credential is available, ask for it immediately and pause; do not retry auth failures. Continue only independent local tasks.
...
13. When DeepSource auth is available, compare grouped issues and issue occurrences for the pushed commit against the pre-push baseline; if either count increased, fetch grouped issue details and repeat the fix/test/push verification loop.
