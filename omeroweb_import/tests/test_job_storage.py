...
def test_env_and_batch_helpers_sanitize_and_clamp(monkeypatch):
    monkeypatch.setenv(job_storage.UPLOAD_BATCH_FILES_ENV, "batch=12 files")

    assert job_storage.get_env_int("MISSING_BATCH_SIZE", 5, 1, 10) == 5
    assert job_storage.get_env_int(job_storage.UPLOAD_BATCH_FILES_ENV, 5, 1, 10) == 10
...
def test_compatibility_helpers_only_start_when_thresholds_are_met(monkeypatch):
    ...
    pending_entries = job_storage.get_compatibility_pending_entries(job)
    assert [entry["status"] for entry in pending_entries] == ["uploaded", "uploaded"]
    assert job_storage._compatibility_pending_entries(job) == pending_entries
...
def test_refresh_job_status_handles_uploading_sem_edx_and_terminal_states():
    uploading = {"files": [{"status": "pending"}]}
    sem_edx = {
        "files": [{"status": "uploaded", "compatibility_skip": True}],
        "special_upload": "sem_edx_spectra",
    }
    incompatible = {"files": [], "compatibility_status": "incompatible"}
    errored = {"files": [], "compatibility_status": "error", "job_id": _job_id()}
    compatible = {"files": [], "compatibility_status": "compatible"}
    checking = {"files": []}

    assert job_storage.refresh_job_status(uploading)["status"] == "uploading"
    refreshed_sem_edx = job_storage.refresh_job_status(sem_edx)
    assert refreshed_sem_edx["compatibility_status"] == "compatible"
    assert refreshed_sem_edx["status"] == "ready"
    assert job_storage.refresh_job_status(incompatible)["status"] == "awaiting_confirmation"
    assert job_storage.refresh_job_status(errored)["status"] == "ready"
    assert job_storage.refresh_job_status(compatible)["status"] == "ready"
    assert job_storage.refresh_job_status(checking)["status"] == "checking"


def test_get_job_path_and_safe_job_id_reject_invalid_values(tmp_path):
    assert job_storage.safe_job_id(_job_id()) is True
    assert job_storage.safe_job_id("not-a-job-id") is False
    assert job_storage.get_job_path(_job_id(), tmp_path) == tmp_path / f"{_job_id()}.json"


def test_load_job_reads_with_lock_and_falls_back_after_lock_contention(tmp_path, monkeypatch):
...
def test_save_job_retries_after_lock_exception_and_persists_timestamp(tmp_path, monkeypatch):
    payload = {"job_id": _job_id(), "status": "ready"}
    ...
    saved = json.loads((tmp_path / f"{_job_id()}.json").read_text(encoding="utf-8"))
    assert saved["status"] == "ready"
    assert isinstance(saved["updated"], float)
    assert attempts["count"] == 2


def test_save_job_returns_false_after_exhausting_retries(tmp_path, monkeypatch):
    payload = {"job_id": _job_id(), "status": "ready"}
...
def test_robust_update_job_updates_atomically_and_handles_corrupt_json(tmp_path):
    path = tmp_path / f"{_job_id()}.json"
    path.write_text(json.dumps({"job_id": _job_id(), "status": "checking"}), encoding="utf-8")
...
def test_append_helpers_store_timestamped_messages(monkeypatch):
