"""
Import workflow orchestration - compatibility checking and import processing.
"""
...
from ...utils.file_helpers import get_jobs_root, get_upload_root
from ...views.core_functions import _safe_relative_path
from omero_plugin_common.omero_helpers import get_id as _get_id
from ..jobs.job_storage import (
    ...
    robust_update_job as _update_job_internal,
    save_job as _save_job_internal,
    should_start_compatibility_check as _should_start_compatibility_check,
)
from ..omero.dataset_service import _dataset_name_for_path
from ..omero.import_service import (
    _append_job_error,
    _append_job_message,
...
def _save_job(job_dict):
    return _save_job_internal(job_dict, get_jobs_root())


def _update_job(job_id: str, update_fn):
    return _update_job_internal(job_id, update_fn, get_jobs_root())
...
def _check_import_compatibility(
    ...
    cli_home = get_plugin_tmp_dir("omeroweb-import") / ".omero-cli-home"
        ...
        with lock:
                ...
                with ThreadPoolExecutor(
                    ...
                    for future in as_completed(futures):
                        ...
                        if result.get("status") == "imported":
                            rel_path = result.get("rel_path") or entry.get(
                                "relative_path"
                            )
                            ...
                            if rel_path:
                                _append_job_message(
                                    job, messages.imported_file(rel_path)
                                )
            ...
            if job.get("special_upload") == "sem_edx_spectra" and sem_edx_associations:
                try:
                    ...
                    if not conn:
                        ...
                        try:
                            ...
                            if create_figures_attachments or create_figures_images:
                                from ..omero.sem_edx_parser import (
                                    ...
                                    if not _validate_session(conn):
                                        ...
                                        try:
                                            try:
                                                ...
                                                logger.debug(
                                                    "Suppressed non-fatal exception in workflow_service.py",
                                                    exc_info=exc,
                                                )
                                            conn = _open_service_connection(
                                                host, port, group_id=job.get("group_id")
                    ...
                    logger.exception(
                        "SEM EDX txt attachment failed for job %s.", job_id
                    )
