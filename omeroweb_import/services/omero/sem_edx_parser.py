...
class GeneticLabelPlacer:
            ...
            for j, gene_j in enumerate(chromosome.genes):
                ...
                for px1, py1 in self.peak_positions[gene_i.label_id]:
                    for px2, py2 in self.peak_positions[gene_j.label_id]:
                        if lines_cross(
                            px1, py1, gene_i.x, gene_i.y, px2, py2, gene_j.x, gene_j.y
                        ):
    ...
    def evolve(self) -> Chromosome:
        ...
        population.extend(
            [self.generate_random_chromosome() for _ in range(self.population_size - 1)]
        )
...
def genetic_label_placement(
    ...
    for symbol, peaks in symbol_groups.items():
        ...
        for i in range(1, len(peaks)):
            ...
            y_diff_ratio = abs(peaks[i][1] - y_current_avg) / max(
                peaks[i][1], y_current_avg, 1.0
            )
    ...
    # Measure labels
    label_specs = []
    for idx, (center_energy, spectrum_y, symbol, peak_energies) in enumerate(
        merged_labels
    ):
...
def create_edx_spectrum_plot(
    ...
    axes_bbox = BBox(
        axes_bbox_raw.x0, axes_bbox_raw.y0, axes_bbox_raw.x1, axes_bbox_raw.y1
    )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "Suppressed non-fatal exception in sem_edx_parser.py", exc_info=exc
                )
            return None
