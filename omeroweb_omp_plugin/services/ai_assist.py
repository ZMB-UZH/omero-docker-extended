...
def _extract_cohere_response_text(payload):
    """Handle extract cohere response text."""
    if not payload:
        return None
    if payload.get("text"):
...
def _split_filename_with_regex(regex, filename):
    """Handle split filename with regex."""
...
def _build_parse_prompt(filenames, custom_instructions=""):
    ...
    prompt = (
        ...
        f"You MUST output EXACTLY {len(sample)} lines — one line per filename.\n"
        "\n"
        "Example with 2 input filenames and 2 output lines:\n"
        "Input filenames:\n"
        "- 10444-ec-01-sa-01-sc-01-20x\n"
        "- sample-cond-ctrl-rep-3-ch-DAPI\n"
        "Output (2 lines):\n"
        "10444,01,01,01,20x\n"
        "ctrl,3,DAPI\n"
        "\n"
        ...
        "- EACH line must be on its OWN line separated by a newline\n"
        "- Output ONLY comma-separated values per line\n"
        "- No headers, no explanations, no quotes, no code fences\n"
        "\n"
        f"Filenames ({len(sample)} total — output {len(sample)} lines):\n"
        f"{list_block}\n"
    )
...
def generate_ai_parsed_values(
    ...
    """Handle generate ai parsed values."""
    provider = (provider or "").strip().lower()
