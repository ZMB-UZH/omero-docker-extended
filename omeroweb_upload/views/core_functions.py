...
def _run_omero_cli(cmd, timeout=300):
    """
    Run OMERO CLI command with timeout protection.
    ...
    CRITICAL FIX: Prevents indefinite hangs when OMERO CLI encounters issues
    """
    try:
        ...
        logger.error(
            "OMERO CLI command timed out after %d seconds: %s",
            timeout,
            ' '.join(str(c) for c in cmd)
        )
        # Return failed result that mimics subprocess.CompletedProcess
        return subprocess.CompletedProcess(
            args=cmd,
            returncode=1,
            stdout="",
            stderr=f"Command timed out after {timeout} seconds. The file may be corrupted, "
                   f"in an unexpected format, or the OMERO server may be experiencing issues."
        )
...
def _import_file(conn, session_key: str, host: str, port: int, path: Path, dataset_id=None, timeout=300):
    ...
    result = _run_omero_cli(cmd, timeout=timeout)
