...
# Read one environment assignment from a file without exporting unrelated keys. Inputs: env file path, key, output variable name. Output: command status and the resolved value in the requested variable.
read_env_assignment_from_file() {
    local env_file_path="${1:?BUG: read_env_assignment_from_file requires a path}"
    local target_key="${2:?BUG: read_env_assignment_from_file requires a key}"
    local __result_var="${3:?BUG: read_env_assignment_from_file requires output variable}"
    local env_line=""
    local env_key=""
    local env_value=""
    local resolved_value=""

    if ! is_shell_variable_name "${target_key}"; then
        echo "ERROR: Invalid environment variable name requested from ${env_file_path}: ${target_key}" >&2
        return 2
    fi
    if ! is_shell_variable_name "${__result_var}"; then
        echo "ERROR: read_env_assignment_from_file output variable is invalid: ${__result_var}" >&2
        return 2
    fi
    if [ ! -r "${env_file_path}" ]; then
        return 1
    ...
    while IFS= read -r env_line || [ -n "${env_line}" ]; do
        case "${env_line}" in
            ...
            [A-Za-z_]*=*)
                ...
                printf -v "${__result_var}" '%s' "${resolved_value}"
                return 0
                ;;
            *)
                ;;
...
write_compose_dot_env() {
    ...
    local dot_env_template_path="${REPO_ROOT_DIR}/.env_example"
    local template_assignments=""

    if ! template_assignments="$(render_compose_dot_env_template_assignments "${dot_env_path}" "${dot_env_template_path}")"; then
        return 1
    fi
...
# Render generated .env assignments from the tracked .env template.
# Inputs: deployment .env path and template path. Output: assignment lines.
render_compose_dot_env_template_assignments() {
    local dot_env_path="${1:?BUG: render_compose_dot_env_template_assignments requires .env path}"
    local template_path="${2:?BUG: render_compose_dot_env_template_assignments requires template path}"
    local env_line=""
    local env_key=""
    local env_value=""
    ...
    if [ ! -r "${template_path}" ]; then
        echo "ERROR: Missing generated .env template: ${template_path}" >&2
        return 1
    fi
    ...
    while IFS= read -r env_line || [ -n "${env_line}" ]; do
        case "${env_line}" in
            ...
            [A-Za-z_]*=*)
                ...
                env_value="${env_line#*=}"
                if ! is_shell_variable_name "${env_key}"; then
                    echo "ERROR: Invalid environment variable name in ${template_path}: ${env_key}" >&2
                    return 1
                fi
                ;;
            *)
...
# Preserve older deployment env files that predate BUILDX_DATA_PATH by deriving
# the same data-root-relative path used by the current installation template.
...
discover_container_default_id_or_die() {
    ...
    if [ -z "${configured_user}" ]; then
        if [ -n "${passwd_user_name}" ]; then
            discover_uid_gid_from_passwd_or_die "${image}" "${passwd_user_name}" "${id_flag}"
            return $?
        fi
