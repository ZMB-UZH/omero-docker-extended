...
- Before every release, obtain the exact GitHub and Docker tags; never infer or auto-increment versions. Require a matching `CHANGELOG.md`, automated disclosure validation, and human public-safety review. Keep notes curated and concise: include notable user/operator impact, compatibility, upgrade actions, and brief verification; omit internal, agent, commit-by-commit, and exhaustive-test detail.
...
- Honor the routing doc's numeric caps before broadening scope; batch bounded read-only work, reuse fresh evidence, never rerun unchanged checks without a changed input or hypothesis, and use `.agents/skills/` when a skill matches.
