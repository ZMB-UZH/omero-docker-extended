  ...
  describe("Enhanced Search HTML help page", () => {
    it("renders compact help with exact interface screenshots", async () => {
      ...
      const screenshots = Array.from(
        document.querySelectorAll(".tools-help-screenshot img"),
      );

      expect(page).toBeTruthy();
      expect(document.title).toBe("Tools help");
      expect(document.querySelector(".admin-tools-title").textContent).toBe(
        "Tools help",
      );
      ...
      expect(
        getComputedStyle(document.querySelector(".tools-help-panel"))
          .borderLeftWidth,
      ).toBe("4px");
      expect(
        getComputedStyle(document.querySelector(".tools-help-panel")).display,
      ).toBe("grid");
      expect(
        getComputedStyle(
          document.querySelector(".tools-help-panel--troubleshooting"),
        ).borderLeftColor,
      ).toBe("rgb(217, 119, 6)");

      const screenshotPanels = Array.from(
        document.querySelectorAll(".tools-help-panel"),
      ).filter((panel) => panel.querySelector(".tools-help-screenshot"));
      for (const panel of screenshotPanels) {
        const copyBox = panel
          .querySelector(".tools-help-copy")
          .getBoundingClientRect();
        const screenshotBox = panel
          .querySelector(".tools-help-screenshot")
          .getBoundingClientRect();
      ...
      expect(
        document.querySelector('img[src$="enhanced-search-results.png"]')
          .naturalHeight,
    ...
    it("keeps the help copy concise and user-facing", async () => {
      ...
      expect(document.querySelectorAll(".tools-help-page button")).toHaveLength(
        1,
      );
      ...
      expect(document.body.textContent).toContain(
        "Saved searches belong to your user account.",
      );
    ...
    it("closes the help window and collapses the tool section without persistence", async () => {
      ...
      expect(
        section.classList.contains("tools-help-tool-section--collapsed"),
      ).toBe(true);

      await installHelpPage();
      expect(
        document
          .querySelector("[data-tools-help-toggle]")
