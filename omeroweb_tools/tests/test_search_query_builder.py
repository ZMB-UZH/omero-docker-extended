...
def test_build_omero_fulltext_query_uses_prefix_matching_for_terms():
    ...
    Inputs: tools-service fixtures. Output: fails on regressions in build OMERO fulltext query uses prefix matching for terms.
    """
...
def test_build_omero_fulltext_query_preserves_exact_phrases():
    """Check that build OMERO fulltext query preserves exact phrases remains stable.

    Inputs: tools-service fixtures. Output: fails on regressions in build OMERO fulltext query preserves exact phrases.
    """
    assert (
...
def test_build_postgres_prefix_tsquery_uses_prefix_and_phrase_operators():
    ...
    Inputs: tools-service fixtures. Output: fails on regressions in build postgres prefix tsquery uses prefix and phrase operators.
    """
    assert (
...
def test_build_query_helpers_ignore_non_search_punctuation():
    ...
    Inputs: tools-service fixtures. Output: fails on regressions in build query helpers ignore non search punctuation.
    """
...
def test_build_query_helpers_drop_single_numeric_fragments_from_decimals():
    ...
    Inputs: tools-service fixtures. Output: fails on regressions in build query helpers drop single numeric fragments from decimals.
    """
    assert (
