...
- The README top badge row is generated by `tools/update_readme_badges.py`; keep it limited to active repository-native status surfaces. The README DeepSource badge may include the `token=` URL parameter copied from the repository's DeepSource Settings > Badges page.
  Treat that exact badge URL parameter as a badge-rendering parameter, not as an API key or private credential, unless new evidence proves otherwise.
- Security scanning is GitHub-native only. Do not search for, create, restore, or edit `.deepsource.toml`; DeepSource repo-file configuration is retired.
