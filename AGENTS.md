...
- Never add AI authorship or agent visibility to tracked artifacts. Do not add AI names, AI contributor entries, AI co-author trailers, agent tags, or “generated by” attributions in commit messages, source headers, OMERO script metadata, docs, changelogs, release notes, UI text, or contributor lists. Keep automation invisible unless the human maintainer explicitly asks otherwise.
- Do not rely on unverified assumptions. Sanity-check factual claims, code-path expectations, and test results against local code, direct command output, or other directly observed evidence before presenting them as true; if something cannot be verified, say so explicitly.
