...
- Do not search for, create, restore, or edit `.deepsource.toml`; DeepSource counts need explicit credentials. GitHub HTTPS Git operations require a PAT/credential manager, never an account password; use `tools/git_push_with_pat.py`. If a GitHub PAT or DeepSource API key is needed and unavailable, ask immediately and pause; do not retry auth failures except local tasks.
- After every push, verify GitHub workflows are green and compare DeepSource grouped issues and issue occurrences for the pushed commit against the pre-push baseline when DeepSource auth is available.
  If either count increased, fetch grouped issue details, fix the regression root cause, rerun targeted tests, and repeat post-push verification.
...
- For broad repo navigation, the CocoIndex Code gate is mandatory: check for a `cocoindex-code` MCP server or tool first, then use `.agents/skills/cocoindex-code-search/` as semantic routing before exact `rg`. Use direct `rg` first only for precise string, symbol, scanner-count, or already-small searches.
  ...
  If CocoIndex starts a cold semantic index, tell the user once that the first search can take several minutes and later searches reuse the external cache. It indexes text-decodable mirrored files through CocoIndex Code 0.2.32; do not claim binary semantic search, add repo-specific language rewrites/file-type exclusions, or use `--lang` on mixed-language files unless proven safe.
  After MCP install or launcher changes, prove stdio `initialize`, `list_tools`, and raw protocol probes with `python3 tools/cocoindex_agent_search.py mcp-smoke`. Use `mcp-smoke --include-search` only against an existing active index; MCP smoke must not build or refresh one.
