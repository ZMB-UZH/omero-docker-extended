...
- Identity audits must check authors, committers, `Co-authored-by` trailers, and GitHub anonymous contributors (`contributors?anon=1`) from fresh branch-head fetches; GitHub PR-head refs are managed snapshots and must be reported separately. Non-AI commit identities must be real human GitHub identities or actual human author names with real email addresses, never host/local placeholders or fake emails.
...
- Use Ruff as the canonical Python formatter and lint gate; host `ruff` must match the repo-pinned version before verification.
...
- The README top badge row is generated by `tools/update_readme_badges.py`; keep it limited to active repository-native status surfaces. Treat the README DeepSource badge `token=` URL parameter as badge-rendering data, not as an API key or private credential, unless new evidence proves otherwise.
- Committed security scanning gates are GitHub-native; do not search for, create, restore, or edit `.deepsource.toml`; DeepSource counts are external API inventory only with explicit credentials. If a GitHub PAT or DeepSource API key is needed and unavailable, ask immediately and pause; do not retry auth failures except independent local tasks.
- After every push, verify GitHub workflows are green and compare DeepSource grouped issues and issue occurrences for the pushed commit against the pre-push baseline when DeepSource auth is available. If either count increased, fetch grouped issue details, fix the regression root cause, rerun targeted tests, and repeat the post-push verification.
