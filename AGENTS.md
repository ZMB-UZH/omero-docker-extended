...
- `tools/run_local_workflow_gates.py` installs Python-backed workflow tools from the same hash-pinned requirement files used by GitHub Actions and runs the locally reproducible workflow gates.
  ...
  Pass every controllable SARIF report through `tools/sarif_result_guard.py` before upload; never call code scanning healthy until the aggregate zero-delta job succeeds and the live default-branch inventory matches the documented three-rule Scorecard baseline.
