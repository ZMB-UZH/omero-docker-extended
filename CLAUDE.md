...
- If the user explicitly asks for lower-token replies, use the opt-in `caveman` skill. It changes reply style only and never changes routing, tool choice, verification scope, or uncertainty handling. Drop back to normal detail whenever safety, sequencing, or ambiguity matters.
