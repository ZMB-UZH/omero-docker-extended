...
- `CONFIG_omero_ldap_new_user_group` (in `env/omeroserver.env`) should be set when LDAP is enabled to avoid fallback to OMERO's built-in default group behavior. Static values (for example `users_ldap`) are now validated at startup and bootstrapped automatically if missing. Dynamic LDAP expressions beginning with `:` are passed through unchanged and are not auto-created because they resolve memberships at login time.
