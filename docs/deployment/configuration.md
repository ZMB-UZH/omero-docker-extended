...
- LDAP bind and directory settings (`CONFIG_omero_ldap_urls`, `CONFIG_omero_ldap_username`, `CONFIG_omero_ldap_password`, `CONFIG_omero_ldap_base`, `CONFIG_omero_ldap__user__filter`) must be set in `env/omero_secrets.env` when `CONFIG_omero_ldap_config=true`.
