...
- CrowdSec installs exactly two local bouncer packages at startup (`crowdsec-firewall-bouncer-iptables` and `crowdsec-nginx-bouncer`) and skips reinstallation when they are already present.
