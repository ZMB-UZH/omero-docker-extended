...
and mirrors the compose-interpolated secret variables (`OMERO_DB_PASS`,
`OMP_PLUGIN_DB_PASS`) so manual `docker compose` commands resolve required
...
If `.env` is missing, or if you run compose commands from a different working
directory, pass all three env files explicitly:

```bash
docker compose --env-file installation_paths.env --env-file env/omero_secrets.env --env-file env/omeroserver.env <command>
```

Once `.env` has been generated by `installation/installation_script.sh`, plain `docker compose <command>` also works from the repository root.
...
docker compose --env-file installation_paths.env --env-file env/omero_secrets.env --env-file env/omeroserver.env stop
...
docker compose --env-file installation_paths.env --env-file env/omero_secrets.env --env-file env/omeroserver.env logs -f omeroweb
