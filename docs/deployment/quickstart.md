...
If `.env` is missing, or if you run compose commands from a different working directory, pass both env files explicitly:

```bash
docker compose --env-file installation_paths.env --env-file env/omero_secrets.env <command>
```
