...
| Add docs-drift guardrails for service-count and topology facts | `README.md`, `ARCHITECTURE.md`, `AGENTS.md`, and `docs/references/docker-compose-llms.txt` now agree on `21 Compose services`, `19` default runtime containers, and `20` with `crowdsec`, but that accuracy still depends on manual upkeep plus a narrow regression test. | Topology drift would mislead operators and future automation. |
