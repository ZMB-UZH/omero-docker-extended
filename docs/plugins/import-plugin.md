...
- **Multi-resolution OME-NGFF zarr flattening**: OMERO's rendering engine (`BfPixelBuffer`) does not correctly handle multi-resolution OME-NGFF zarrs — it may read from a lower resolution level instead of the full resolution at thumbnail/render time, causing `DimensionsOutOfBoundsException`. To prevent this, the plugin automatically detects OME-NGFF zarrs with multiple resolution levels (via the `multiscales.datasets` array in `.zattrs`) and strips all lower-resolution arrays before import, keeping only the highest-resolution dataset. This flattening preserves 100% of the full-resolution pixel data; only the pre-computed downsampled pyramid levels are removed (OMERO generates its own pyramid as needed for large images). Zarrs using `bioformats2raw.layout` are not affected — OMERO handles their resolution hierarchy correctly. Flattening operates on a temporary copy; the original uploaded file is never modified.
