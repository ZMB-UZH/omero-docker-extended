...
- **Zarr permanent-store path hardening**: before `omero zarr import`, the plugin copies the directory into a unique sanitized child directly under `OMERO_ZARR_STORE_ROOT` instead of relying on nested `group/user/...` parent directories. This avoids permission regressions when stale intermediate directories were created with the wrong owner while still keeping the permanent store readable by OMERO server components.
