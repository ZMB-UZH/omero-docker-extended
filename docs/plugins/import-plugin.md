...
- **Native Zarr routing and validation**: the plugin routes every Zarr image layout actually supported by the installed `omero-cli-zarr` runtime through `omero zarr import`, not just one hand-picked example. Today that means pure OME-NGFF image stores and `bioformats2raw.layout=3` image stores. The router reads `.zattrs` the same way `omero-cli-zarr` does (including `ome`-wrapped metadata), validates the referenced primary array metadata before registration, and rejects unsupported recognized layouts such as OME-Zarr plates or Zarr v3 stores against a `.zarray`-based render stack.
- **Pinned native Zarr toolchain**: the `omeroweb` image installs `omero-cli-zarr` and `bioformats2raw` from explicit compose-driven build args (`OMERO_CLI_ZARR_VERSION`, `BIOFORMATS2RAW_VERSION`) so native Zarr behavior stays reproducible and upgrades remain deliberate.
- **Zarr pre-flight scan and automatic re-compression**: the Bio-Formats dry-run (`omero import -f`) is still used for non-native `.zarr` paths and for all non-zarr imports. Native `omero zarr import` candidates bypass that scan and go through the native validator instead. When the Bio-Formats path finds 0 groups (for example gzip/zstd compression unsupported by JZarr), the plugin automatically re-compresses the Zarr data to zlib using `numcodecs`, chunk-by-chunk, without loading whole arrays into memory.
- **Native OME-NGFF pass-through**: supported pure OME-NGFF image stores are handed to `omero zarr import` without rewriting their multiscale layout. The current stack renders staged raw multiscale NGFF correctly, while flattening the same store before managed-repository registration can break thumbnail generation on the managed-path code path. The plugin still validates native candidates before import, but it no longer mutates supported OME-NGFF layout metadata just to force a synthetic single-scale view.
...
| `CONFIG_omero_managed_dir` | Managed-repository directory name under `OMERO_DATA_DIR` |
