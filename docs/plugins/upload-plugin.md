...
- Request-path dataset preparation now prefers the background logical import-unit plan that OMERO/Bio-Formats already produced, so dataset creation stays aligned with grouped/package imports across formats instead of relying on a `.zarr`-specific heuristic.
