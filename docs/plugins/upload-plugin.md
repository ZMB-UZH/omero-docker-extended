...
- **Import success validation**: after every `omero import` call, the plugin verifies that stdout contains at least one imported OMERO object ID (Image, Fileset, Plate, etc.). If the CLI exits 0 but creates no objects, the import is reported as an error with a clear message instead of a false success. Conversely, if the CLI exits non-zero but stdout still contains created object IDs (e.g. because the server committed the Image before a post-import step failed), the import is treated as successful with a warning logged — preventing false failures for imports that actually completed.
