...
- **Zarr pre-flight scan and automatic re-compression**: before importing any `.zarr` directory, the plugin runs `omero import -f` (Bio-Formats dry-run) to verify the format is importable. If Bio-Formats finds 0 groups (e.g. gzip/zstd compression unsupported by JZarr), the plugin automatically re-compresses the zarr data to zlib (supported by JZarr) using the Python `zarr` library, then imports the converted copy. The temporary re-compressed zarr is deleted after import. This allows pure OME-NGFF zarrs (e.g. from `ome-zarr-py`) to be imported into OMERO's managed repository without user intervention.
- **Import success validation**: after every `omero import` call, the plugin verifies that stdout contains at least one imported OMERO object ID (Image, Fileset, Plate, etc.). If the CLI exits 0 but creates no objects, the import is reported as an error with a clear message instead of a false success.
- **Progress bar accuracy**: the orange import progress bar uses the higher of two signals (real `/proc/{pid}/io` monitoring and time-based asymptotic estimate) so the bar never gets stuck at a low percentage for large imports. On browser refresh, progress bars jump instantly to the server-reported position without replaying the CSS animation.
