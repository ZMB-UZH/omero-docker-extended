...
- `10-server-bootstrap.sh` validates writable directories, explicitly prepares the OMERO CLI user temp directory (`$HOME/tmp`) before any OMERO CLI call, configures certificates, and schedules async operations (job-service user creation, script registration) that do not block server startup.
