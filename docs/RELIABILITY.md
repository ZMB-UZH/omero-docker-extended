...
- `10-server-bootstrap.sh` validates writable directories, auto-detects the OMERO CLI binary (with optional `OMERO_BIN` override), explicitly prepares a clean OMERO CLI runtime temp namespace under `${OMERO_TMP_PATH}/${OMERO_CLI_USER}/tmp/runtime*` before any OMERO CLI call, and now also removes stale legacy `omero_${OMERO_CLI_USER}` lock namespaces directly under `${OMERO_TMP_PATH}/${OMERO_CLI_USER}/tmp` so repeated reinstall/update cycles cannot leave `.lock` files owned by the wrong runtime user. It then repoints the legacy OMERO lock-file path to that clean runtime slot and schedules async operations (job-service user creation, script registration, and managed-repository shared-prefix normalization) that do not block server startup.
- `installation/installation_script.sh` now preserves per-subtree ownership under `OMERO_TMP_PATH` during reinstall/update runs: the temp root and non-server plugin subtrees stay owned by `omero-web`, while the full `${OMERO_TMP_PATH}/omero-server` namespace is recursively restored to `omero-server`. This avoids recursive ownership drift of stale OMERO.server lock trees during repeated `github_pull_project_bash` or `github_pull_private_project_bash` reinstall workflows.
