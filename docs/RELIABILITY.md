...
- `10-server-bootstrap.sh` validates writable directories, auto-detects the OMERO CLI binary (with optional `OMERO_BIN` override), explicitly prepares a clean OMERO CLI runtime temp namespace under `${OMERO_TMP_PATH}/${OMERO_CLI_USER}/tmp/runtime*` before any OMERO CLI call, repoints the legacy OMERO lock-file path to that clean runtime slot so stale lock files from older runs cannot disable `Processor-0`, then configures certificates and schedules async operations (job-service user creation, script registration, and managed-repository shared-prefix normalization) that do not block server startup.
