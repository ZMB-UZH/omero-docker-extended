...
- Use `cocoindex-code-search` only as a shared host-side routing aid: check for
  ...
  same-name MCP entries instead of adding duplicates, use a repository-relative
  MCP command, verify MCP changes with
  `python3 tools/cocoindex_agent_search.py mcp-smoke`, and keep MCP search
  read-only against an existing active index. Cold install, mirror, daemon, and
  ...
  0.2.31 to include every Git-visible mirrored file pattern; CocoIndex indexes
  ...
  opt-in path. Always confirm with exact `rg` before editing.
