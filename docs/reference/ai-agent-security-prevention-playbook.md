...
8. After pushing, confirm GitHub workflows are green, refresh the live GitHub
   code-scanning count, and when DeepSource auth plus repository access are
   available compare grouped issues plus issue occurrences against the pre-push
   baseline. If DeepSource is skipped or unavailable because of credentials,
   subscription, repository access, or API availability, report that status and
   continue the rest of the verification instead of treating the skipped
   DeepSource check as a blocking failure. Do not infer closure from local
   reasoning alone.
