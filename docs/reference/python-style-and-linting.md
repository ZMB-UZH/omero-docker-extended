...
- The workflow uses pinned GitHub Actions and a pinned Ruff release (`0.16.6`).
- The Mypy workflow restores and stores the `pip` download cache using the hash-pinned `.github/requirements/tests-ci.txt` and `.github/requirements/mypy-ci.txt` lockfiles as its cache key, installs both dependency sets, then runs `python3 tools/mypy_check.py`. The current Mypy CI pin is `2.3.1`.
...
the deployed Python 3.11 web image therefore retains OME-Zarr 0.18.0.
Keep the production example pin unchanged until a compatible web base is
validated. NGFF changes need both the newer CI tests and real checks against
