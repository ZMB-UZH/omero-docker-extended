# AI Agent Task Contracts

Conditional detail for [AGENTS.md](../../AGENTS.md). Read only the section
triggered by the current action, not this whole file at session start.
These requirements remain mandatory when their trigger applies.
...
- Do not create feature branches, PR branches, temporary remote branches, or draft PRs for routine coding, verification, workflow checks, or scanner checks. If one is created accidentally, preserve the work and return to the resolved default branch. Obtain fresh approval for each branch deletion or PR closure before cleanup; accidental creation does not authorize deletion.
...
Non-example deployment files include `env/omero_secrets.env`; its values must
never be printed or mutated without the exact one-off authorization above.
...
- If live verification shows an ignored host env file has an outdated value, report the key name and required action only, never the configured value without exact authorization. Change it only as a narrow host-local edit after explicit user approval, preserving all unrelated values.
...
Maintain a verification ledger for the stable final tree. Reuse fresh evidence
from unchanged inputs; do not repeat a passing check merely for reassurance.
Changed code, configuration, fixtures, dependencies, or runtime identities
invalidate affected checks. The full required final-tree matrix still applies.
...
- Do not search for, create, restore, or edit `.deepsource.toml`; DeepSource counts need explicit credentials and can be unavailable for auth, subscription, or access.
  GitHub HTTPS Git operations require a PAT/credential manager, never an account password; use `tools/git_push_with_pat.py`. If a GitHub PAT is unavailable, ask immediately and pause for input; do not retry auth failures except local tasks.
...
Read `docs/reference/ai-agent-runtime-playbook.md` before live operations.
Before ANY file-sync or rsync operation, run `python3 tools/env_safety_guard.py check`
and `python3 tools/env_safety_guard.py backup`. NEVER use `rsync --delete` or any
tree-replacement command on the working directory. Sync through a disposable clone;
follow `docs/operations/repository-sync-safety.md`.
Discover live host bindings, container IDs, and OMERO.web virtualenv paths from
Compose/container state. After the first host sandbox miss, use container-network
probes. Never run OMERO CLI as `root` inside OMERO containers.
